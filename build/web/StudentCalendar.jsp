...
<%@page import="uta.cse4361.beans.ScheduleAppointmentBean"%>
...
<html>
    <head>    
        ...
        <jsp:setProperty name="newAppt" property="type" param="ty pe" /> 
                            ...
                            <h3>Appointment Summary</h3>
                                <div>                         
                                    <form name="appointmentSummary" role="form">
                                        ...
                                        <input class="form-control" name="semester" id="semester" value="<%= request.getParameter("semester") %>" readonly="readonly" ></textarea>
                                         </div>
                                          <div class="form-group">
                                            <label for="description">Priority</label> 
                                        <input class="form-control" name="priority" id="priority" value="<%= request.getParameter("priority") %>" readonly="readonly"></textarea>
                                          </div>
                                           <div class="form-group">
                                            <label for="description">Courses Enrolled</label> 
                                        <input class="form-control" name="subEnrolled" id="subEnrolled" value="<%= request.getParameter("subEnrolled") %>" readonly="readonly"></textarea>
                                           </div>
                                            <div class="form-group">
                                            <label for="description">Appointment Time Required</label> 
                                        <input class="form-control" name="timeReq" id="timeReq" value="<%= request.getParameter("timeReq") %>" readonly="readonly"></textarea>
                                            </div>
                                    ...
                                    <%if(timeSubmitted){
                                           newAppt.setSemester(request.getParameter("semester"));
                                           newAppt.setTimeReq(request.getParameter("timeReq"));
                                           newAppt.setSubEnrolled(request.getParameter("subEnrolled") );
                                           newAppt.setPriority(request.getParameter("priority"));
                                           int count = newAppt.getCountOfAppointments(); 
                                           String result = newAppt.scheduleAppointment();
                                            int count1 = newAppt.getCountOfAppointments();
                                            
                                            if(count == count1){
                                               out.println( "Can not schedule more than one appointment");
